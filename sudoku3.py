...
##for i in range(10):
##    tk.Label(root,text='%d'%i,width=5).grid(row=0,column=i)

frame=tk.Frame(root, bg ='dim gray')
root.title("Sudoku")
...
for line in range(0, 9):
    for col in range(0,9):
        entries[(line, col)] = tk.StringVar()
        e = tk.Entry(frame, width = 3, fg = 'firebrick1', justify='center', font=("Times", 10, "bold"), textvariable=entries[(line, col)])
        if col == 3 or col == 6:
            e.grid(row=line, column=col, padx = (2, 0))
        if line == 3 or line == 6:
            e.grid(row=line, column=col, pady =(2, 0))
        else:
            e.grid(row=line, column=col)
...
def insert_message():
   tkMessageBox.showinfo("Insert", "The numbers were saved")

def callback1():
    global n, grid
    for line in range(0, 9):
        for col in range(0,9):
            if entries[(line, col)].get() == '':
                grid[line][col] = 0
            else:
                grid[line][col] = int(entries[(line, col)].get())
    insert_message()
...
def callback2():
    ...
    while get_known_nr(known_nr) < 81:
        ...
        for line in range(0, 9):
            for col in range(0,9):
                entries[(line, col)].set(grid[line][col])
             
        print 'numere cunoscute', get_known_nr(known_nr)
        n += 1

b1 = tk.Button(frame, text="Insert", command=callback1)
b1.grid(row=10, column = 0, columnspan =2)


b2 = tk.Button(frame, text="Start", command=callback2)
b2.grid(row=10, column = 2, columnspan = 2)



frame.grid(rowspan=9, columnspan=9, padx = 5, pady=30)
root.mainloop()
